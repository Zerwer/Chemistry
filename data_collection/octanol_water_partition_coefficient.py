...
def partition_coefficient(data):
    ...
    for point in part_data['Information']:
        if 'est' not in str(point['Value'] and 'ph' not in str(point['Value'])):
            logP = float(re.findall(r'[-+]?[0-9]*\.?[0-9]+', str(point['Value']))[0])
            if logP < 500:
                owpc.append(logP)
...
collect('data/OctanolWaterPartitionCoefficient/list.txt', 'owpc.txt', partition_coefficient)